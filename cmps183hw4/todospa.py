...
def view_to_db_edit_task(task):
    ''' convert view edit task to db row '''
    return (task['taskdescription'], view_to_db_status(task['status']))
...
def view_get_tasks(filter):
    ...
    # get db_tasks
    rows = db_get_tasks_by_status(filter)
...
def view_edit_task(edit_task):
    ''' create new task:
        store task in db
        retrieve, convert to view, and return stored task
    '''
    # convert task data from vm to db
    db_edit_task = view_to_db_edit_task(edit_task)
    # create record in db
    taskid = db_edit_task(db_edit_task)
    # confirming record was correctly added
    row = db_get_task(taskid)
    result = db_to_view_task(row) if row else {'error': "task insertion failed"}
    return result
...
def view_delete_task(vm_task_id):
    ''' updates status to vm_status for task with taskid '''
    taskid = vm_task_id['taskid']
    db_id = taskid
    db_delete_task(db_id)
...
def edit_task():
    ''' record a new task '''
    edit_task = unpack_task(request)
    task = view_edit_task(edit_task)
    return pack_task(task)
...
def delete():
    ...
    updated_task = view_delete_task(task_data)
